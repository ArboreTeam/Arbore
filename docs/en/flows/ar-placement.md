...
The HUD keeps the camera as the primary surface: the top bar contains back, undo/redo, and validation with compact buttons, while the bottom dock fits on one row. In placement mode, the dock shows the **Floor/Wall/Ceiling** modes, the share photo/video capture, and plant add, then automatically folds back to the active mode to free up the camera. When a plant is selected, that same dock replaces placement actions with compact edit mode: plant name, exit edit mode, rotate, zoom +/−, and delete. The lux indicator is no longer shown; light level stays only as an internal signal for legacy auto-placement heuristics. The share button opens a dedicated Camera-inspired capture mode: hidden HUD, hidden placement reticle, central shutter, compact **Photo/Video** selector, recording counter for video, and last-capture thumbnail. The thumbnail opens a local AR gallery persisted for the garden; captures can be deleted, and tapping a capture then opens a separate edit/share page with retake, sharing, and an optional Arbore logo overlay.

The AR catalog opens as an opaque full-screen page and receives the garden's `GardenWizardDTO`. **Suited to my space** is selected by default and ranks plants from data that is actually available: space type, estimated sunlight, child/pet safety, container planting, drainage, wind, indoor humidity, nearby heat, and available height. Missing data is never invented: the plant remains marked **Check** and is not included in the suited selection. **All plants** keeps the badges, sorts the strongest matches first, and asks for confirmation before placing a plant identified as less suitable. Physical **Floor/Wall/Ceiling** compatibility remains a separate hard filter.

Search is complemented by three shortcuts (**Flowering**, **Compact**, **Easy care**) and a preference page organized into five families: goal, plant type, colors and foliage, dimensions, then care. Each family opens a visual subpage whose options adapt to the space type and the Floor/Wall/Ceiling placement mode. Multiple choices within one family use OR; active families are combined with AND. The sticky action previews the result count before applying the preferences.

The page makes it clear that **Arbore adaptation** remains active: light, climate, wind, drainage, humidity, child/pet safety, available height, and planting mode come from the garden and are not asked for again as preferences. Manual filters only express user intent (for example adding flowers, creating privacy, attracting pollinators, choosing a color, or limiting care). Tapping a card badge opens the positive reasons and warnings; the botanical detail remains available from there or from the context menu.

To keep interaction responsive, every plant's textual and structured traits are indexed once outside SwiftUI's render path. The index stays in memory for fifteen minutes, garden-specific suitability is computed once when the catalog opens, and search and filters reuse the existing sort. Thumbnails check the disk cache first and then the remote render. A PNG without the current design signature is rejected: the catalog photo appears immediately as a preview while a corrected thumbnail is regenerated locally from the USDZ.
