...
| `Services/LLMService.swift` | Conversational assistant (`actor`): relays the history + message to `POST /chat`. **Formerly `GeminiService`** — the provider's name no longer appears in the app, the backend picks it (#555). Consults `AIProcessingPreference` before any send. |
...
| `Config/ConsentDefaults.swift` | Single source of truth for GDPR consent defaults (#218) — **privacy by default**: every free consent is `false`. `ai` is the exception (`true`) because it is not a consent but a feature preference on a contractual basis, and therefore does **not** appear in `initialSnapshot`. |
| `Config/AIProcessingPreference.swift` | Centralised read of the “AI processing” setting (key `privacy_ai`, default `ConsentDefaults.ai`). Consulted by **both** send sites — health scan and assistant (#549). Off: the scan falls back to local colorimetry, the assistant becomes unavailable. |
| `Services/RemoteConfigService.swift` | Loads `GET /config` at launch (#236), caches it for offline use, and exposes it both as an `ObservableObject` (UI) **and** as a synchronous singleton (model layers). Every accessor has a hardcoded fallback: the app stays functional when the backend is unreachable. |
| `Thumbnail/PlantThumbnailCache.swift` + `PlantThumbnailGenerator.swift` | Plant thumbnails: queued offscreen rendering (`@MainActor` generator, one render at a time) and a versioned disk cache (`version = "v22"`). The version is shared by the local cache **and** the public URL: bumping it invalidates both the on-device files and Cloudflare's cached response. |
