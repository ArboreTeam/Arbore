...
| `Services/LLMService.swift` | Assistant conversationnel (`actor`) : relaie l'historique + le message vers `POST /chat`. **Ex-`GeminiService`** — le nom du fournisseur ne figure plus côté app, c'est le backend qui choisit (#555). Consulte `AIProcessingPreference` avant tout envoi. |
...
| `Config/ConsentDefaults.swift` | Source unique des valeurs par défaut des consentements RGPD (#218) — **privacy by default** : tout consentement libre est `false`. `ai` fait exception (`true`) car ce n'est pas un consentement mais une préférence de fonctionnalité en base contractuelle, et n'apparaît donc **pas** dans `initialSnapshot`. |
| `Config/AIProcessingPreference.swift` | Lecture centralisée du réglage « Traitement IA » (clé `privacy_ai`, défaut `ConsentDefaults.ai`). Consulté par les **deux** points d'envoi — scan de santé et assistant (#549). Éteint : le scan se rabat sur la colorimétrie locale, l'assistant devient indisponible. |
| `Services/RemoteConfigService.swift` | Charge `GET /config` au lancement (#236), le met en cache pour l'hors-ligne, et l'expose comme `ObservableObject` (UI) **et** comme singleton synchrone (couches modèle). Chaque accesseur a un repli codé en dur : l'app reste fonctionnelle backend injoignable. |
| `Thumbnail/PlantThumbnailCache.swift` + `PlantThumbnailGenerator.swift` | Vignettes de plantes : rendu hors écran mis en file (générateur `@MainActor`, un rendu à la fois) et cache disque versionné (`version = "v22"`). La version est partagée par le cache local **et** l'URL publique : la bumper invalide les fichiers sur l'appareil et la réponse mise en cache par Cloudflare. |
