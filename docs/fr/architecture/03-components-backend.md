...
> **Note `OPENAI_API_KEY`** : consommée uniquement par l'AI Generator lorsque ce fournisseur est sélectionné, jamais par le backend Go. **Note `PORT`** : définit le port d'écoute du serveur Go ; Docker le mappe sur le même port côté hôte par défaut.
