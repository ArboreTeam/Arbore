...
class PrivacySettingsIntegrationTests: XCTestCase {
    ...
    private func ensureBackendIsReachableOrSkip() throws {
        guard let url = URL(string: "\(AppConfig.baseURL)/health") else {
            throw XCTSkip("PrivacySettings integration tests skipped: invalid backend URL \(AppConfig.baseURL)")
        }
        ...
        if !isReachable {
            throw XCTSkip("PrivacySettings integration tests skipped: backend is unreachable at \(AppConfig.baseURL)")
        }
    }
    ...
    func testSyncConsentToBackend_MultipleConsents_ShouldSyncAll() {
        ...
        // Forcer l'exécution du runloop pour permettre aux requêtes async de se compléter
        ...
        // Assertion simple - on vérifie juste que le test s'est exécuté sans crash
