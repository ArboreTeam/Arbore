...
The AR catalog opens as an opaque full-screen page and receives the garden's `GardenWizardDTO`. **Suited to my space** is selected by default and uses sourced botanical profiles plus facts that are actually available: indoor/outdoor use, hardiness, direct sun, known safety, pot volume, drainage, watering, wind, humidity, and dimensions. Critical constraints run before ranking. The three verdicts are **Suitable**, **Probably compatible**, and **Not suitable**. An unaudited profile can never be labelled Suitable; it may remain in the default scope as probably compatible when at least one known criterion matches. **All plants** keeps the badges and asks for confirmation before placing a plant marked Not suitable. Physical **Floor/Wall/Ceiling** compatibility remains a separate hard filter.
...
The page makes it clear that **Arbore adaptation** remains active: light, available climate, wind, drainage, watering, humidity, known safety, dimensions, and planting mode come from the garden and are not asked for again as preferences. Manual filters only express user intent. Tapping a card badge separately shows positive reasons, conflicts, and missing information.
