...
Le catalogue AR s'ouvre en page plein écran opaque et reçoit le `GardenWizardDTO` du jardin. L'onglet **Adaptées à mon espace**, sélectionné par défaut, utilise le profil botanique sourcé et les données réellement disponibles : intérieur/extérieur, rusticité, soleil direct, sécurité connue, volume du pot, drainage, arrosage, vent, humidité et dimensions. Les contraintes critiques sont appliquées avant le classement. Les trois verdicts sont **Adaptée**, **Probablement compatible** et **Non adaptée**. Une fiche non auditée ne peut jamais recevoir « Adaptée » ; elle peut rester dans la sélection par défaut comme probablement compatible si au moins un critère connu est favorable. L'onglet **Toutes les plantes** conserve ces badges et demande confirmation avant de poser une plante non adaptée. La compatibilité physique **Sol/Mur/Plafond** reste un filtre dur indépendant.
...
La page rappelle que l'**adaptation Arbore** reste active : lumière, climat disponible, vent, drainage, arrosage, humidité, sécurité connue, dimensions et mode de plantation proviennent du jardin et ne sont donc pas redemandés comme préférences. Les filtres manuels expriment uniquement l'intention de l'utilisateur. Toucher le badge ouvre séparément les raisons favorables, incompatibilités et informations manquantes.
