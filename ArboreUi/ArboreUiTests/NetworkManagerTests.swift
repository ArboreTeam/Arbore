...
class NetworkManagerIntegrationTests: XCTestCase {
    ...
    private func ensureBackendIsReachableOrSkip() throws {
        guard let url = URL(string: "\(AppConfig.baseURL)/health") else {
            throw XCTSkip("NetworkManager integration tests skipped: invalid backend URL \(AppConfig.baseURL)")
        }
        ...
        if !isReachable {
            throw XCTSkip("NetworkManager integration tests skipped: backend is unreachable at \(AppConfig.baseURL)")
        }
    }
